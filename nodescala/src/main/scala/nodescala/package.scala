...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
      ...
      def allrec(fs: List[Future[T]], res:List[T]):Unit = {
        if (fs == Nil) {
          p success res
        }else {
          fs.head.onComplete {
            case Success(value) => println("Hallo")
              allrec(fs.tail, res ::: List(value))
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      val p = Promise[T]
      def anyRec(fs: List[Future[T]], allFutures:List[Future[T]]): Unit = {
        if (fs == Nil) anyRec(allFutures, allFutures)
        if(fs.head.isCompleted){
          fs.head onComplete{
            case Success(value) => p success(value)
            case Failure(t) => p failure (t)
          }
        }else{
          anyRec(fs.tail, allFutures)
        }
      }
      anyRec(fs, fs)
      p.future
     ...
     */
      def run()(f: CancellationToken => Future[Unit]): Subscription = {
        ...
        val Future = f(ct.cancellationToken)
        ct
      }
