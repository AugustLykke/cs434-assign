...
class FunSetSuite extends FunSuite {
  ...
  trait TestSets {
    ...
    def s0To10 (x:Int) = x > 0 && x <= 10
    def s5To10 (x:Int) = x >=5 && x <= 10
  }
  ...
  test("diff contains only elements in set1, that are not in set 2 ") {
    new TestSets {
      val s = diff(s0To10, s5To10)
      assert(contains(s, 4), "diff 1")
      assert(!contains(s, 7), "diff 2")
      assert(!contains(s, 11), "diff 3")
    }
  }

  test("filter returns subset which holds for filter") {
    new TestSets{
      val s = filter(s0To10, x => x % 2 == 0) 
      assert(contains(s, 4), "filter 1")
      assert(contains(s, 6), "filter 1")
      assert(!contains(s, 7), "filter 2")
